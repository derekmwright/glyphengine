...
`travel` is the depth divided by `dot(N, V)`, not the depth itself â€” looking
...
empty mesh â€” a surface with nothing under it is almost always a mistake in the
...
instead of discarding it â€” water still has to be occluded by terrain in front
...
| `Normal` | shallow water colour â€” the real normal comes from the wave derivatives |
