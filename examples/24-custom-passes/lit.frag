...
// Slot 0 set by SetShaderTarget: the previous write of the smoothed light map.
...
void main() {
    ...
    // Only the ground receives the world-XZ projection; the boxes still use
    // ordinary lighting and cast the engine's directional shadows.
    vec2 fieldUV = fragWorldPos.xz / 40.0 + 0.5;
    if (fragWorldPos.y < 0.01 && N.y > 0.9 &&
        all(greaterThanEqual(fieldUV, vec2(0))) && all(lessThanEqual(fieldUV, vec2(1))))
        lit *= 1.0 + 2.5 * texture(applicationField, fieldUV).r;
